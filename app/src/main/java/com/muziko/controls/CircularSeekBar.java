...
public class CircularSeekBar extends View {
	...
	private static final int DEFAULT_CIRCLE_COLOR = Color.parseColor("#e4e4e4");
	private static final int DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(255, 0, 156, 218);
	...
	public void setProgress(int progress) {
		if (mProgress != progress) {
			...
			if (mOnCircularSeekBarChangeListener != null) {
				mOnCircularSeekBarChangeListener.onProgressChanged(this, progress);
			}
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				if (((touchEventRadius >= innerRadius) && (touchEventRadius <= outerRadius)) && ((cwDistanceFromPointer <= pointerRadiusDegrees) || (ccwDistanceFromPointer <= pointerRadiusDegrees))) {
					...
					invalidate();
					if (mOnCircularSeekBarChangeListener != null) {
						mOnCircularSeekBarChangeListener.onStartTrackingTouch();
					}
					...
					if (mOnCircularSeekBarChangeListener != null) {
						mOnCircularSeekBarChangeListener.onStartTrackingTouch();
						mOnCircularSeekBarChangeListener.onProgressChanged(this, mProgress);
					}
					mUserIsMovingPointer = true;
					lockAtEnd = false;
					lockAtStart = false;
	...
	public interface OnCircularSeekBarChangeListener {
		...
		void onProgressChanged(CircularSeekBar circularSeekBar, int progress);
