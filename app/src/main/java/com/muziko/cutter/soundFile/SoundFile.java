...
public class SoundFile {
    ...
    // A SoundFile object should only be created using the static methods createActivityListener() and record().
    private SoundFile() {
	}
	...
	public static SoundFile create(String fileName,
			...
			throws
			java.io.IOException, InvalidInputException, IBIException {
	...
	private void ReadFile(File inputFile)
		...
		System.out.println("FILE TYPE " + mFileType);
		...
		System.out.println("mSAMPLE RATE " + mSampleRate);
		if (mSampleRate <= 24000) {

			throw new IBIException("Bad File Type");
		}
		int expectedNumSamples =
				(int) ((format.getLong(MediaFormat.KEY_DURATION) / 1000000.f) * mSampleRate + 0.5f);
		System.out.println("EXPECTED SAMPLE NUM " + expectedNumSamples);
		...
		// only once.
		int checkValSala = 1 * (1 << 20);
		mDecodedBytes = ByteBuffer.allocate(checkValSala);
		Boolean firstSampleData = true;
		int chckIbI = codec.dequeueInputBuffer(100);

		System.out.println("CODEC FILE VALUE " + chckIbI);
		while (true) {
	...
	private void WriteFile(File outputFile, float startTime, float endTime)
		...
		while (true) {
			...
			if (!done_reading && inputBufferIndex >= 0) {
				if (num_samples_left <= 0) {
                    ...
                    // bufferSize is a hack to createActivityListener a stereo file from a mono stream.
		...
		try {
            ...
            Log.e("Ringdroid", "Failed to createActivityListener the .m4a file.");
	...
	private void DumpSamples(String fileName) {
		...
		try {
            ...
            Log.w("Ringdroid", "Failed to createActivityListener the sample TSV file.");
