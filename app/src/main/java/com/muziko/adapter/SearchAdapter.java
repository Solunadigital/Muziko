...
public class SearchAdapter extends SelectableAdapter<RecyclerView.ViewHolder> implements Filterable, FastScrollRecyclerView.SectionedAdapter {
	...
	public SearchAdapter(Context context, ArrayList<QueueItem> listData, int type, boolean showArtwork, String tag, SearchRecyclerListener listener) {
		super();
	...
	public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		...
		if (viewType == 1) {
			view = LayoutInflater.from(mContext).inflate(R.layout.search_section_header, parent, false);
		...
		if (item != null) {
			switch (item.type) {
				case MyApplication.TRACKHEADER:
					...
					headerViewHolder.icon.setImageResource(R.drawable.menu_ringtone);
					break;
				case MyApplication.ARTISTHEADER:
					headerViewHolder = (HeaderViewHolder) viewHolder;
					headerViewHolder.sectiontitle.setText(MyApplication.ARTISTS);
					headerViewHolder.icon.setImageResource(R.drawable.menu_artist);
					break;
				case MyApplication.ALBUMHEADER:
					headerViewHolder = (HeaderViewHolder) viewHolder;
					headerViewHolder.sectiontitle.setText(MyApplication.ALBUMS);
					headerViewHolder.icon.setImageResource(R.drawable.menu_album);
					break;
				default:
	...
	private class CustomFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				result.count = 0;
				result.values = new ArrayList<QueueItem>();
				setFilterCount(0);
			}
