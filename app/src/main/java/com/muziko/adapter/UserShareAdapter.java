...
public class UserShareAdapter extends SelectableAdapter<UserShareAdapter.AdapterQueueHolder> implements Filterable, FastScrollRecyclerView.SectionedAdapter {
	private final Context mContext;
	private final ArrayList<Person> filts;
	...
	public UserShareAdapter(Context context, ArrayList<Person> listData, Collection<Person> selected, String tag, ShareListener listener) {
		super();
	...
	public AdapterQueueHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		View view = LayoutInflater.from(mContext).inflate(R.layout.adapter_user_share_item, parent, false);
	...
	public void onBindViewHolder(final AdapterQueueHolder holder, int position) {
		...
		if (item != null) {
			...
			for (Person person : selected) {
				if (person.getUid().equals(item.getUid())) {
					holder.selected.setVisibility(View.VISIBLE);
					holder.layoutMain.setBackgroundColor(ContextCompat.getColor(mContext, R.color.share_contact_overlay));
				}
			}
	...
	public void sortTitleLowest() {
		...
		Collections.sort(items, (s1, s2) -> s1.getDisplayName().compareToIgnoreCase(s2.getDisplayName()));
