...
public class StorageAdapter extends SelectableAdapter<StorageAdapter.AdapterQueueHolder> implements Filterable, FastScrollRecyclerView.SectionedAdapter {
    ...
    public StorageAdapter(Context context, ArrayList<QueueItem> listData, int type, boolean showArtwork, String tag, RecyclerItemListener listener) {
        super();
    ...
    public AdapterQueueHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case 1:
                ...
                break;
            default:
                view = LayoutInflater.from(mContext).inflate(R.layout.adapter_storage_item, parent, false);
                break;
        }
    ...
    public void onBindViewHolder(final AdapterQueueHolder holder, int position) {
        ...
        if (item != null) {
            ...
            if (item.order == R.id.drives) {
                ...
                if (item.folder) {
                    if (showHidden) {
                        holder.hiddenOverlay.setVisibility(StorageFolderRealmHelper.getFolder(item.storage, item.data).isHiddenFolder() ? View.VISIBLE : GONE);
                    }
                    ...
                    if (storageMode == CloudManager.StorageMode.DRIVES) {
                        holder.textDesc.setText(item.folder_path);
                    } else {
                        holder.textDesc.setText("");
                    }
                    holder.imageThumb.setImageResource(R.drawable.folder_icon);
