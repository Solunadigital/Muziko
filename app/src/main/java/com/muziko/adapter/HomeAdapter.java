...
import com.muziko.interfaces.HomeTabRecyclerItemListener;
 ...
 */
public class HomeAdapter extends SelectableAdapter<RecyclerView.ViewHolder> implements Filterable, FastScrollRecyclerView.SectionedAdapter {
    private final int HOME = 99;
    ...
    private final HomeTabRecyclerItemListener homeListener;
    private String mtag;
    private int storage;
    ...
    public HomeAdapter(Context context, ArrayList<QueueItem> listData, int type, String tag, TrackRecyclerItemListener listener, HomeTabRecyclerItemListener homeListener) {
        super();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        if (viewType == HOME) {
            view = LayoutInflater.from(mContext).inflate(R.layout.adapter_home_item, parent, false);
            return new HeaderViewHolder(mContext, view, homeListener);
        } else {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (viewHolder.getItemViewType() == HOME) {
            ...
            headerViewHolder.libraryDetailText.setText(FirebaseManager.Instance().getFirebaseLibraryTracks().size() + " songs");
            headerViewHolder.playlistsDetailText.setText(PlaylistRealmHelper.loadAll().size() + " playlists");
        } else {
            AdapterQueueHolder holder = (AdapterQueueHolder) viewHolder;
...
//    }
    ...
    public static class HeaderViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        private final CardView libraryCard;
        private final CardView storageCard;
        private final CardView sharingCard;
        private final CardView playlistsCard;
        private final TextView libraryDetailText;
        private final TextView playlistsDetailText;
        private final TextView showAllText;
        private final HomeTabRecyclerItemListener listener;
        private final Context context;

        public HeaderViewHolder(final Context context, final View view, final HomeTabRecyclerItemListener listener) {
            super(view);
            ...
            storageCard = view.findViewById(R.id.storageCard);
            sharingCard = view.findViewById(R.id.sharingCard);
            playlistsCard = view.findViewById(R.id.playlistsCard);
            libraryDetailText = view.findViewById(R.id.libraryDetailText);
            playlistsDetailText = view.findViewById(R.id.playlistsDetailText);
            showAllText = view.findViewById(R.id.showAllText);

            libraryCard.setOnClickListener(this);
            storageCard.setOnClickListener(this);
            sharingCard.setOnClickListener(this);
            playlistsCard.setOnClickListener(this);
        ...
        public void onClick(View v) {
            if (listener != null) {
                if (v == libraryCard) {
                    listener.onLibraryClicked();
                } else if (v == storageCard) {
                    listener.onStorageClicked();
                } else if (v == sharingCard) {
                    listener.onSharingClicked();
                } else if (v == playlistsCard) {
                    listener.onPlaylistsClicked();
                } else if (v == showAllText) {
                    listener.onShowAllClicked();
                }
            }
        }
    }
