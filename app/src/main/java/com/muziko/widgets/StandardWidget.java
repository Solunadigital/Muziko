...
public class StandardWidget extends AppWidgetProvider {
		...
		if (rows == 1) {
			if (columns <= 3) {
				return new RemoteViews(context.getPackageName(),
						R.layout.widget_standard_low_narrow);
			} else {
				return new RemoteViews(context.getPackageName(),
						R.layout.widget_standard_low);
			}
		} else {
			return new RemoteViews(context.getPackageName(),
					R.layout.widget_standard_high);
    ...
    private void setListeners(Context context, RemoteViews view) {
        ...
        Intent playsong = new Intent(SongService.NOTIFY_PLAY_SONG);
...
//
//				queueItem = PlayerConstants.QUEUE_SONG;
//				PrefsManager.Instance().setStandardWidgetCurrentSong(context, queueItem.data);
//			} else {
//				MyApplication.loadQueue(context);
//				String data = PrefsManager.Instance().getStandardWidgetCurrentSong(context);
...
//			}
			...
			AsyncJob.doOnMainThread(() -> Picasso.with(context).load(url).resize(MyApplication.IMAGE_LARGE_SIZE, MyApplication.IMAGE_LARGE_SIZE).centerInside().error(R.mipmap.placeholder).into(remoteViews, R.id.imageViewAlbumArt, appWidgetIds));
