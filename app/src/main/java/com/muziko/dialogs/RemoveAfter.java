...
public class RemoveAfter implements View.OnClickListener {
	private static final String TAG = RemoveAfter.class.getSimpleName();
	...
	public void open(Context context, QueueItem queueItem) {
		...
		createDialog = new MaterialDialog.Builder(mContext)
				...
				.positiveText("Save").onPositive((dialog, which) -> {
					...
					item.copyQueue(queueItem);
					item.hash();

                    final long level = PrefsManager.Instance().getQueueLevel();
                    item.level = level;

					int index = PlayerConstants.QUEUE_LIST.size();
					item.order = index;
					item.removeafter = removeAfter;
                    ...
                    AppController.toast(context, "Song added to queue!");
