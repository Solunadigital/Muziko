...
class BackgroundDataJob implements AsyncJob.OnBackgroundJob {
	...
	public void doOnBackground() {
		try {
			...
			if (!data.isEmpty()) {
				salutInstance.dataReceiver.activity.runOnUiThread(() -> salutInstance.dataReceiver.dataCallback.onDataReceived(data));
			}
		} catch (Exception ex) {
