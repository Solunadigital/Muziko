...
public class Salut implements WifiP2pManager.ConnectionInfoListener {
	...
	public Salut(Context context, SalutDataReceiver dataReceiver, SalutServiceData salutServiceData, SalutCallback deviceNotSupported) {
		WifiManager wifiMan = (WifiManager) dataReceiver.context.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
	...
	public void setOnDeviceUnregisteredCallback(SalutHostCallback callback) {
	...
	public ArrayList<SalutDevice> getReadableRegisteredDevices() {
		ArrayList<SalutDevice> registeredNames = new ArrayList<>(registeredClients.size());
		for (SalutDevice device : registeredClients) {
			registeredNames.add(device);
		}
	...
	public void onConnectionInfoAvailable(final WifiP2pInfo info) {
        /* This method is automatically called when we create to a device.
                ...
                group -> {
                    if (isRunningAsHost && !registrationIsRunning) {
                        ...
                        if (serviceRequest == null) {
                            ...
                            Log.e(
                                    Salut.TAG,
                                    "This device is still connected to an old host for some reason. A forced disconnect will be attempted.");
                            forceDisconnect();
                        }
	...
	private void startUploadTask(Context context, final SalutDevice device, QueueItem queueItem, @Nullable final SalutUploadCallback salutUploadCallback) {

		final String uuid = UUID.randomUUID().toString();

		SalutFileUploader salutFileUploader = new SalutFileUploader(context, device, this, queueItem, uuid, salutUploadCallback);

		MyApplication.shareUploaderList.put(uuid, salutFileUploader);
		salutFileUploader.execute();
		...
		AsyncJob.doInBackground(() -> {
			try {
				...
				while (isRunningAsHost || thisDevice.isRegistered) {
					...
					SalutFileDownloader salutFileDownloader = new SalutFileDownloader(mContext, Salut.this, uuid, dataListener);

					MyApplication.shareDownloaderList.put(uuid, salutFileDownloader);
					salutFileDownloader.execute();
	...
	public void registerWithHost(final SalutDevice device, @Nullable SalutRegisterCallback onRegistered, @Nullable final SalutRegisterCallback onRegistrationFail) {
	...
	public void startUpload(Context context, final SalutDevice device, final QueueItem queueItem, @Nullable final SalutUploadCallback salutUploadCallback) {
		startUploadTask(context, device, queueItem, salutUploadCallback);
	}
        ...
        manager.cancelConnect(
                ...
                new WifiP2pManager.ActionListener() {
                    ...
                    public void onSuccess() {
                        Log.v(TAG, "Attempting to cancel create.");
                    }
                    ...
                    public void onFailure(int reason) {
                        Log.v(
                                TAG,
                                "Failed to cancel create, the device may not have been trying to create.");
                    }
                });
	...
	private void connectToDevice(final SalutDevice device, final SalutRegisterCallback onFailure) {
        ...
        manager.connect(
                ...
                new WifiP2pManager.ActionListener() {
                    ...
                    @Override
                    public void onSuccess() {
                        Log.d(TAG, "Attempting to create to another device.");
                        lastConnectedDevice = device;
                    }

                    @Override
                    public void onFailure(int reason) {
                        onFailure.onRegisterFailed("Failed to create to device");
                        Log.e(TAG, "Failed to create to device. ");
                    }
                });
	}
	...
	private void createService(final SalutHostCallback onSuccess, final SalutHostCallback onFailure) {
		...
		manager.addLocalService(channel, serviceInfo, new WifiP2pManager.ActionListener() {
					...
					if (group != null) {
							...
							public void onSuccess() {
								...
								manager.createGroup(channel, new WifiP2pManager.ActionListener() {
									...
									public void onFailure(int reason) {
										...
										if (onFailure != null)
											onFailure.onHostError("Failed to start listening. Trying again");
									}
								});
							}
						...
						manager.createGroup(channel, new WifiP2pManager.ActionListener() {
							...
							public void onFailure(int reason) {
								Log.e(TAG, "Failed to create group. Reason :" + reason);
								if (onFailure != null)
									onFailure.onHostError("Failed to create group");
							}
						});
					}
				});
			...
			public void onFailure(int error) {
				...
				if (onFailure != null)
                    onFailure.onHostError("Failed to createActivityListener " + thisDevice.serviceName + " : Error Code: " + error);
            }
		});
	}

	public void unregisterClient(@Nullable SalutRegisterCallback onSuccess, @Nullable SalutRegisterCallback onFailure, boolean disableWiFi) {
	...
	public void startNetworkService(@Nullable SalutHostCallback onDeviceRegisteredWithHost, @Nullable SalutHostCallback onSuccess, @Nullable SalutHostCallback onFailure) {
	...
	private void setupDNSResponders() {
        ...
        *double check if the device is running our service, and then we go ahead and pull that specific information from it and put it into an Map.*/
		WifiP2pManager.DnsSdTxtRecordListener txtRecordListener = (serviceFullDomainName, record, device) -> {
        ...
        *that was passed in early is also called.*/
		WifiP2pManager.DnsSdTxtRecordListener txtRecordListener = (serviceFullDomainName, record, device) -> {
			...
			if (record.containsValue(thisDevice.serviceName)) {
				...
				foundDevices.add(foundDevice);
				if (callContinously) {
					onDeviceFound.onDeviceFound(foundDevice);
				} else {
					if (!firstDeviceAlreadyFound) {
						onDeviceFound.onDeviceFound(foundDevice);
						firstDeviceAlreadyFound = true;
		...
		manager.discoverServices(channel, new WifiP2pManager.ActionListener() {
			...
			public void onFailure(int arg0) {
				...
				if (arg0 == WifiP2pManager.P2P_UNSUPPORTED)
					deviceNotSupported.onError("This device ddoes not support Wifi Direct");
