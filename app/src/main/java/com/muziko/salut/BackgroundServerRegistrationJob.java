...
class BackgroundServerRegistrationJob implements AsyncJob.OnBackgroundJob {
		...
		try {
			...
			if (!clientDevice.isRegistered) {
				...
				if (salutInstance.onDeviceRegisteredWithHost != null) {
					salutInstance.dataReceiver.activity.runOnUiThread(() -> salutInstance.onDeviceRegisteredWithHost.onClientConnected(finalDevice));
				}
				...
				for (final SalutDevice registered : salutInstance.registeredClients) {
					if (registered.serviceAddress.equals(clientSocket.getInetAddress().toString().replace("/", ""))) {
						...
						if (salutInstance.onDeviceRegisteredWithHost != null) {
							salutInstance.dataReceiver.activity.runOnUiThread(() -> salutInstance.onDeviceRegisteredWithHost.onClientDisconnected(registered));
						}
