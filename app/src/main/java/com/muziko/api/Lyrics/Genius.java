...
public class Genius {
	...
	public static ArrayList<Lyrics> search(String query) {
		...
		try {
			...
			Connection connection = Jsoup.connect(queryURL.toExternalForm())
                    .header("Authorization", "Bearer " + BuildConfig.GENIUS)
                    .timeout(0)
					.ignoreContentType(true);
			Document document = connection.userAgent(NetworkHelper.USER_AGENT).get();
	...
	public static Lyrics fromURL(String url, String artist, String title) {
		...
		String text;
		try {
			lyricsPage = Jsoup.connect(url).userAgent(NetworkHelper.USER_AGENT).get();
