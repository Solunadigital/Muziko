...
import static com.muziko.manager.MuzikoConstants.PRODUCT_ID;
...
public class Premium {
	...
	// default constructor
	public Premium(final Activity activity, onUpdatedListener listener) {
		mContext = activity;
		mListener = listener;
	}
	...
	public void buyPremium() {
		...
		if (isAvailable) {
			billingProcessor.purchase(mContext, PRODUCT_ID);
		} else {
	...
	private class PremiumListener implements BillingProcessor.IBillingHandler {
		...
		public void onPurchaseHistoryRestored() {
			...
			if (products != null && products.indexOf(PRODUCT_ID) != -1) {
                PrefsManager.Instance().setPremium(true);
            } else {
                PrefsManager.Instance().setPremium(false);
            }
		...
		public void onBillingError(int errorCode, Throwable error) {
			...
			switch (errorCode) {
				...
				case Constants.BILLING_ERROR_OTHER_ERROR: //110;
					if (billingProcessor.loadOwnedPurchasesFromGoogle() && billingProcessor.isPurchased(PRODUCT_ID)) {
        ...
        public void onBillingInitialized() {
            ...
            //SkuDetails sku = billingProcessor.getPurchaseListingDetails(PRODUCT_ID);
