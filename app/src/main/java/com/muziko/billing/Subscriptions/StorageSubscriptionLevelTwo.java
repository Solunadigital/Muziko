...
public class StorageSubscriptionLevelTwo {

    private final String TAG = StorageSubscriptionLevelTwo.class.getSimpleName();
    private final Activity mContext;
    private BillingProcessor billingProcessor;
    private onUpdatedListener mListener;

    // default constructor
    public StorageSubscriptionLevelTwo(final Activity activity, onUpdatedListener listener) {
        mContext = activity;
        mListener = listener;
    }
    ...
    public void buyProduct() {
        ...
        if (isAvailable) {
            billingProcessor.subscribe(mContext, STORAGE_SUBSCRIPTION_LEVEL_TWO);
        } else {
    ...
    private class PremiumListener implements BillingProcessor.IBillingHandler {
        ...
        public void onProductPurchased(String productId, TransactionDetails details) {
            ...
            if (productId != null && productId.equals(STORAGE_SUBSCRIPTION_LEVEL_TWO) && billingProcessor.isValidTransactionDetails(details)) {
                PrefsManager.Instance().setSubscription(STORAGE_SUBSCRIPTION_LEVEL_TWO);
                ...
                new MaterialDialog.Builder(mContext).theme(Theme.LIGHT).titleColorRes(R.color.normal_blue).negativeColorRes(R.color.dialog_negetive_button).positiveColorRes(R.color.normal_blue).title("Subscription Upgrade").content("Congratulations, You purchased level two").negativeText("OK").show();
            }
        ...
        public void onPurchaseHistoryRestored() {
            ...
            if (products != null && products.indexOf(STORAGE_SUBSCRIPTION_LEVEL_TWO) != -1) {
                PrefsManager.Instance().setSubscription(STORAGE_SUBSCRIPTION_LEVEL_TWO);
            }
        ...
        public void onBillingError(int errorCode, Throwable error) {
            ...
            switch (errorCode) {
                ...
                case Constants.BILLING_ERROR_OTHER_ERROR: //110;
                    if (billingProcessor.loadOwnedPurchasesFromGoogle() && billingProcessor.isPurchased(STORAGE_SUBSCRIPTION_LEVEL_TWO)) {
