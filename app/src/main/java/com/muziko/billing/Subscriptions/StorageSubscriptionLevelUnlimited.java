...
public class StorageSubscriptionLevelUnlimited {

    private final String TAG = StorageSubscriptionLevelUnlimited.class.getSimpleName();
    private final Activity mContext;
    private BillingProcessor billingProcessor;
    private onUpdatedListener mListener;

    // default constructor
    public StorageSubscriptionLevelUnlimited(final Activity activity, onUpdatedListener listener) {
        mContext = activity;
        mListener = listener;
    }
    ...
    public void buyProduct() {
        ...
        if (isAvailable) {
            billingProcessor.subscribe(mContext, STORAGE_SUBSCRIPTION_LEVEL_UNLIMITED);
        } else {
    ...
    private class PremiumListener implements BillingProcessor.IBillingHandler {
        ...
        public void onProductPurchased(String productId, TransactionDetails details) {
            ...
            if (productId != null && productId.equals(STORAGE_SUBSCRIPTION_LEVEL_UNLIMITED) && billingProcessor.isValidTransactionDetails(details)) {
                PrefsManager.Instance().setSubscription(STORAGE_SUBSCRIPTION_LEVEL_UNLIMITED);
                ...
                new MaterialDialog.Builder(mContext).theme(Theme.LIGHT).titleColorRes(R.color.normal_blue).negativeColorRes(R.color.dialog_negetive_button).positiveColorRes(R.color.normal_blue).title("Subscription Upgrade").content("Congratulations, You purchased level four").negativeText("OK").show();
            }
        ...
        public void onPurchaseHistoryRestored() {
            ...
            if (products != null && products.indexOf(STORAGE_SUBSCRIPTION_LEVEL_UNLIMITED) != -1) {
                PrefsManager.Instance().setSubscription(STORAGE_SUBSCRIPTION_LEVEL_UNLIMITED);
            }
