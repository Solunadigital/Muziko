...
import pl.tajchert.buswear.wear.SendAssetToNode;
...
public class EventBus {
    ...
    public void postAssetEvent(Bitmap bitmap, String key) {
        ImageAssetEvent imageAssetEvent = new ImageAssetEvent(bitmap, key);
        postLocal(imageAssetEvent);
    }
    ...
    private void sendEventOverGooglePlayServices(Object event, boolean isSticky) {
        ...
        if (event.getClass().equals(ImageAssetEvent.class)) {
            ImageAssetEvent imageAssetEvent = (ImageAssetEvent) event;
            Asset asset = createAssetFromBitmap(imageAssetEvent.getBitmap());

            try {
                new SendAssetToNode(asset, imageAssetEvent.getKey(), imageAssetEvent.getClass(), context, isSticky).start();
            } catch (Exception e) {
